...
-- 9. job_id 가 'FI_ACCOUNT' 또는 'IT_PROG' 이고, salary 가 8000 이상 인 employees 테이블 을 조회한다.
select *
from employees
where commission_pct is not null;
...
select employee_id, email, salary, NVL(commission_pct, 0) as ncommission_pct
from employees;
...
-- 14. employees 테이블을 employee_id, first_name, last_name 을 조회하되, last_name 의 길 이가 5 이하인 건만 조회한다.
select employee_id, first_name, last_name
from employees
where length(last_name) <= 5;

-- 15. employees 테이블에서 salary 가 5000 이상인 건수를 조회하고 high_salary_cnt 로 표시 한다.
select count(*) as high_salary_cnt
from employees
where salary >= 5000;

-- 16. employees 테이블에서 job_id 가 'IT_PROG' 인 조건에 해당하는 salary 의 전체 합을 구 하고 it_prog_salary_sum 으로 표시한다. 
select sum(salary) as it_prog_salary_sum
from employees
where job_id = 'IT_PROG';

 
-- 17. employees 테이블에서 hire_date가 가장 빠른 값과 가장 늦은 값을 구하고 각각 hire_date_min, hire_date_max 로 표시한다.
select max(hire_date) as hire_date_max, min(hire_date) as hire_date_min
from employees;

-- 18. employees 테이블에서 job_id 와 job_id 별 employee 건수를 조회하고 job_id_employee_cnt 로 표시한다.
select job_id, count(*) as job_id_employee_cnt
from employees
group by job_id;


-- 19. employees 테이블에서 department_id별 salary 의 합을 조회하고 department_id_salary_sum 로 표시한다. 단, department_id 가 없는 건은 제외한다.
select department_id, sum(salary) as department_id_salary_sum
from employees
...
from employees
group by department_id
having sum(salary) <= 5000 and department_id is not null;
